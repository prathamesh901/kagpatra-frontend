...
const PrintingInProgressPage = () => {
	...
	const [printTime, setPrintTime] = React.useState<string | null>(null);

	// Poll backend for print job status
	React.useEffect(() => {
		let interval: NodeJS.Timeout;
		let isMounted = true;
		const pollStatus = async () => {
			try {
				const res = await fetch("http://192.168.2.116:3000/api/print-job-status");
				const data = await res.json();
				if (!isMounted) return;
				if (data.status === "preparing") setStep(0);
				else if (data.status === "printing") setStep(1);
				else if (data.status === "done") {
					setStep(2);
					if (typeof data.printTime === "number") {
						setPrintTime(data.printTime.toFixed(1) + "s");
					}
				}
			} catch (e) {
				// ignore
	...
	// Redirect to confirmation when done
	React.useEffect(() => {
		if (step === 2) {
	...
	}, [step, navigate, uploadedFileName, estimatedCost, printTime]);
	...
	return (
		<div className="min-h-screen flex flex-col bg-white">
			...
			<div className="flex flex-col items-center mt-2 mb-10">
				<div className="flex items-center gap-7 mb-2">
					...
					<div className="flex flex-col items-center transition-all duration-500">
						<div
							className={`size-12 rounded-full flex items-center justify-center border-4 ${
								step >= 0
									? "bg-[#4664EA]/90 border-[#EAEFFF]"
									: "bg-[#f9faff] border-[#ececff]"
					...
					<div
						style={{
							width: 36,
							height: 4,
							background: step > 0 ? "#4664EA" : "#ececff",
						}}
					...
					<div
						style={{
							width: 36,
							height: 4,
							background: step === 2 ? "#22c55e" : "#ececff",
						}}
						className="transition-all duration-700"
					/>
					{/* Step 3: Done */}
			...
			<div className="flex-1 flex flex-col items-center justify-center">
				<div
					className="text-2xl font-semibold text-center mb-4 mt-6 text-black animate-fade-in"
					key={step}
				>
					{stepInfo[step].message}
				</div>
				<div
					className="text-base text-gray-400 text-center mb-8 max-w-xs animate-fade-in"
					key={step + "_desc"}
				>
